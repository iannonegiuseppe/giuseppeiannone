...
export default async function DesignLabHomepage() {
  ...
  // Photo pass: real landscape photograph (5000x2203, well over the
  // 16:9 frame's own need), not CMS-sourced — a static asset under
  // public/design-lab/photos/ (see that folder's own placement note),
  // not Sanity, since this is density's own hardcoded-content block, not
  // a CMS-wired one. object-position lives on .photoFrameImg itself
  // (density.module.scss) — see that rule's own comment for the crop
  // math (subject sits in the right ~62% of the frame; a plain "right
  // center" position keeps the whole subject in view, including where
  // the crossed arm already touches the source photo's own right edge).
      ...
      <div className={densityStyles.root}>
        ...
        <header className={densityStyles.pageHeader}>
          ...
          <p className={densityStyles.pageNote}>
            Questa è la proposta di homepage costruita a partire dai blocchi
            rilavorati e da quelli esistenti.
          </p>
          <p className={densityStyles.pageNote}>
        ...
        <main>
          ...
          <section
            className={densityStyles.credentialsBand}
            aria-label="Credenziali"
          >
            <div
              className={densityStyles.credentialsGrain}
              aria-hidden="true"
            />
            <div className={densityStyles.credentialsInner}>
              <ul className={densityStyles.credentialsList}>
                {credentials.items.map((item) => (
                  <li
                    key={item.detail}
                    className={densityStyles.credentialsItem}
                  >
          ...
          <section
            className={densityStyles.section}
            aria-labelledby="diplomi-block-heading"
          >
            <DiplomiBlock
          ...
          <div className={densityStyles.richDarkWrapContact}>
            <div
              className={densityStyles.richDarkOverlayContact}
              aria-hidden="true"
            />
            <div className={densityStyles.richDarkContactContent}>
              <FinalContactSection
                ...
                googleProfileLabel={
                  homePage?.finalCta?.googleProfileLabel ?? ""
                }
            ...
            <div
              className={densityStyles.richDarkGrainContact}
              aria-hidden="true"
            />
          </div>
